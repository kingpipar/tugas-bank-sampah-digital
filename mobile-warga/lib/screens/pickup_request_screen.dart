...
class _PickupRequestScreenState extends State<PickupRequestScreen> {
  ...
  Future<void> _handleSubmit() async {
    ...
    final request = PickupRequestModel(
      userId: auth.user?.id ?? '',
      estimasiKantong: int.tryParse(_kantongController.text) ?? 1,
      estimasiBerat: double.tryParse(_beratController.text) ?? 0,
      catatan: _catatanController.text.trim(),
      // TODO: Ganti dengan koordinat GPS asli dari device
      latitude: AppConstants.mockLatitude,
      longitude: AppConstants.mockLongitude,
    );

    final success = await pickup.submitPickupRequest(request);
    ...
    } else if (mounted && pickup.errorMessage != null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(pickup.errorMessage!),
